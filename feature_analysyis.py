...
#jos haluu tehä multiclass nii pitää myögemmi laskee keskiarvoje keskiarvot
...
coef_lists = []
for model in models:
  cv_results = cross_validate(model, train_x, train_y, cv=5, scoring='accuracy', return_estimator=True)
  est_coefs = [est.coef_ for est in cv_results['estimator']]

  temp = pd.DataFrame(np.squeeze(np.array(est_coefs)))
  temp = temp.agg(['mean'])
